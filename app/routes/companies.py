...
from app.config.database import get_database  # Import modifié
...
@router.get("/")  # Suppression du / car déjà dans le prefix
async def get_companies(db: AsyncIOMotorDatabase = Depends(get_database)):
    try:
        companies = await db.companies.find().to_list(1000)
        for company in companies:
            company["id"] = str(company.pop("_id"))  # Changer _id en id pour le frontend
        return companies
...
async def create_company(
    ...
    try:
        ...
        if existing_company:
            raise HTTPException(
                status_code=409,  # Code plus approprié pour un conflit
                detail="Cet identifiant SIRET est déjà utilisé par une autre entreprise"
            )

        # Vérifier si l'email est déjà utilisé
        existing_email = await db.companies.find_one({"email": email})
        if existing_email:
            raise HTTPException(
                status_code=409,
                detail="Cette adresse email est déjà utilisée par une autre entreprise"
            )
...
async def send_credentials(data: dict):
    try:
        ...
        print(f"Erreur lors de l'envoi de l'email: {str(e)}")  # Ajout d'un log pour déboguer
        raise HTTPException(status_code=500, detail=str(e))
    

@router.post("/{company_id}/toggle-lock")  # Modification du chemin pour être cohérent
async def toggle_company_lock(
    ...
    try:
        print(f"Tentative de verrouillage pour l'entreprise: {company_id}")  # Debug log
        ...
        print(f"Changement de statut: {company['is_active']} -> {new_status}")  # Debug log
        ...
        print(f"Résultat de la mise à jour: {result.modified_count} document(s)")  # Debug log
        ...
        if not new_status:
            ...
            print(f"Utilisateurs désactivés: {users_result.modified_count}")  # Debug log
        ...
        print(f"Erreur lors du verrouillage: {str(e)}")  # Debug log
        raise HTTPException(
            status_code=500,
...
async def update_company(
    ...
    is_active: bool = Form(...),
    password: Optional[str] = Form(None),  # Nouveau champ optionnel
    logo: Optional[UploadFile] = File(None),
    db: AsyncIOMotorDatabase = Depends(get_database)
):
    try:
